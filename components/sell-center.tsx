...
import {
  ...
  rubles,
  type InventoryItem,
  type LiveInventoryListing,
} from "@/lib/product";

type InventoryResult = {
  ok?: boolean;
  inventory?: InventoryItem[];
  liveListings?: LiveInventoryListing[];
};
...
export default function SellCenter() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    setError(null);
    try {
      const result = await session.callAction("create_listing", {
        itemId: selected.id,
        price: Number(price),
        title,
        description,
  ...
  async function cancel(listingId: string) {
    ...
    setError(null);
    try {
      await session.callAction("cancel_listing", { listingId });
      await loadInventory();
    } catch {
  ...
  if (session.state !== "verified" && !loading) {
    return (
      <div className="authGate">
        <div className="authGateIcon">↗</div>
        <span className="sectionEyebrow">Продажа</span>
        <h1>Твой инвентарь живёт в Telegram</h1>
        <p>Открой Mini App через @{session.botUsername}. После первого входа TradeUP выдаст один стартовый набор, который можно сразу выставить на рынок.</p>
        <button type="button" className="primaryAction" onClick={session.openBot}>Открыть TradeUP</button>
      </div>
    );
  }
  ...
  return (
    <div className="sellPage">
      <div className="pageHeadline">
        <div><span className="sectionEyebrow">Продать</span><h1>Инвентарь</h1><p>Выбери предмет, назначь цену и отправь его на живой рынок.</p></div>
        <div className="inventoryCounter"><strong>{inventory.length}</strong><span>предметов</span></div>
      </div>

      {session.starterGranted > 0 && (
        <div className="starterBanner"><span>🎁</span><div><strong>Стартовый набор получен</strong><p>{session.starterGranted} предмета уже лежат в инвентаре. Это реальные игровые предметы, а не фейковые объявления.</p></div></div>
      )}
      ...
      {!loading && inventory.length === 0 && (
        <div className="emptyPanel"><div className="emptySymbol">□</div><h3>Инвентарь пуст</h3><p>После покупки товары будут появляться здесь и их можно будет перепродавать.</p><Link href="/" className="primaryAction">Искать на рынке</Link></div>
      )}
      ...
      {!loading && inventory.length > 0 && (
        <div className="inventoryGrid">
          {inventory.map((item) => {
            ...
            const meta = categoryMeta[itemType?.category_id ?? ""] ?? { icon: "📦", short: "Предмет" };
            return (
              <article className={`${selectedId === item.id ? "inventoryCard selected" : "inventoryCard"} ${live ? "listed" : ""}`} key={item.id}>
                ...
                {live && (
                  <div className="listedBar"><div><span>На продаже</span><strong>{rubles(live.price)}</strong></div><div><Link href={`/listing/${live.id}`}>Открыть</Link><button type="button" onClick={() => void cancel(live.id)} disabled={submitting}>Снять</button></div></div>
                )}
              </article>
            );
          })}
      ...
      {selected && !activeByItem.has(selected.id) && (
        <form className="sellComposer" onSubmit={submit}>
          <div className="composerHeading"><div><span className="sectionEyebrow">Новое объявление</span><h2>{selected.item_types?.name}</h2></div><button type="button" onClick={() => setSelectedId(null)}>×</button></div>
          ...
          <p className="composerHint">После продажи TradeUP удержит 4% комиссии. Покупатель платит только указанную цену.</p>
        </form>
      )}
    </div>
  );
