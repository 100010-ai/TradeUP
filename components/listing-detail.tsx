...
function Sparkline({ points }: { points: number[] }) {
  ...
  return (
    <svg className="sparkline" viewBox="0 0 100 36" preserveAspectRatio="none" aria-label="История цены">
...
export default function ListingDetail({ id }: { id: string }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const historyResult = await supabase
        ...
        .eq("item_type_id", typed.item_type_id)
  ...
  async function toggleFavorite() {
    ...
    setMessage(null);
    try {
      await session.callAction("toggle_favorite", { listingId: id });
    } catch (error) {
  ...
  async function buy() {
    ...
    try {
      ...
      setMessage(code === "insufficient_funds" ? "Не хватает денег на балансе" : code === "listing_not_active" ? "Этот лот уже купили или сняли" : "Покупка не прошла");
    } finally {
      setActionLoading(false);
    }
  }
  ...
  async function cancel() {
    ...
    try {
      ...
      setMessage("Не удалось снять лот");
    } finally {
      setActionLoading(false);
    }
  }
  ...
  if (!listing) {
    return (
      <div className="emptyPanel">
        ...
        <h3>Лота больше нет на рынке</h3>
        <p>Его могли купить или продавец снял объявление.</p>
        <Link href="/" className="primaryAction">Вернуться на рынок</Link>
      </div>
    );
  }
  ...
  return (
    <div className="listingDetailPage">
      <Link href="/" className="backLink">← Назад к рынку</Link>
      ...
      <section className="detailGrid">
        ...
        <div className="detailInfo">
          <div className="detailTopline">
            <span>{listing.brand ?? listing.category_name}</span>
            <button type="button" className={favorite ? "favoriteRound active" : "favoriteRound"} onClick={() => void toggleFavorite()} disabled={actionLoading} aria-label="В избранное">
              {favorite ? "♥" : "♡"}
            </button>
          </div>
          <h1>{listing.title}</h1>
          <div className="detailPriceRow">
            <strong>{rubles(listing.price)}</strong>
            <span className={delta <= -5 ? "priceSignal good" : delta >= 8 ? "priceSignal high" : "priceSignal"}>{percent(delta, false)} к ориентиру</span>
          </div>
          <p className="detailDescription">{listing.description || "Продавец не добавил описание."}</p>

          <div className="detailFacts">
            <div><span>Состояние</span><strong>{conditionLabel(listing.condition)}</strong></div>
            <div><span>Ориентир</span><strong>{rubles(fair)}</strong></div>
            <div><span>Опубликовано</span><strong>{relativeDate(listing.created_at)}</strong></div>
      ...
      <section className="detailLowerGrid">
        <div className="infoCard sellerCard">
          <span className="sectionEyebrow">Продавец</span>
          <div className="sellerProfileRow">
            <div className="sellerAvatarLarge">{listing.seller_photo_url ? <img src={listing.seller_photo_url} alt="" /> : listing.seller_first_name.charAt(0).toUpperCase()}</div>
            <div><h3>{listing.seller_first_name}</h3><p>{sellerLevel(listing.seller_rating)} · {listing.seller_deals_count} сделок</p></div>
            <div className="ratingPill">★ {listing.seller_rating}</div>
          </div>
          {listing.seller_username && <div className="sellerUsername">@{listing.seller_username}</div>}
        </div>

        <div className="infoCard priceHistoryCard">
          <div className="cardHeadingInline"><div><span className="sectionEyebrow">Цена</span><h3>История предмета</h3></div><strong>{pricePoints.length} точек</strong></div>
          <Sparkline points={pricePoints} />
          <p>График строится только по реальным завершённым сделкам и стартовому ориентиру каталога.</p>
        </div>
      </section>
    </div>
  );
