...
const offerStatusLabel: Record<OfferRow["status"], string> = {
  pending: "Ждёт ответа",
  accepted: "Принято",
  declined: "Отклонено",
  cancelled: "Отменено",
...
export default function DealsCenter() {
  ...
  async function loadAll() {
    const [dealsRaw, offersRaw] = await Promise.all([
      session.callAction("deals"),
      session.callAction("offers"),
    ]);
    const deals = dealsRaw as DealsResult;
    const offers = offersRaw as OffersResult;
    setTrades(deals.trades ?? []);
    setItems(deals.items ?? []);
    setListings(deals.listings ?? []);
    setIncoming(offers.incoming ?? []);
    setOutgoing(offers.outgoing ?? []);
    setOfferListings(offers.listings ?? []);
    setOfferProfiles(offers.profiles ?? []);
  }
  ...
  async function respond(offerId: string, accept: boolean) {
    ...
    setError(null);
    try {
      await session.callAction("respond_offer", { offerId, accept });
      await loadAll();
    } catch (reason) {
      const code = reason instanceof Error ? reason.message : "game_action_failed";
      setError(code === "insufficient_funds" ? "У покупателя уже не хватает денег" : code === "offer_not_pending" ? "Предложение уже закрыто" : "Не удалось обработать предложение");
    } finally {
      setActionId(null);
    }
  }
  ...
  async function cancelOffer(offerId: string) {
    ...
    setError(null);
    try {
      await session.callAction("cancel_offer", { offerId });
      await loadAll();
    } catch {
      setError("Не удалось отменить предложение");
    } finally {
      setActionId(null);
    }
  }
  ...
  if (session.state !== "verified" && !loading) {
    return (
      <div className="authGate compactGate"><div className="authGateIcon">⇄</div><span className="sectionEyebrow">Сделки</span><h1>История твоего оборота</h1><p>Покупки, продажи, торг и реальная маржа привязаны к Telegram-профилю.</p><button type="button" className="primaryAction" onClick={session.openBot}>Открыть TradeUP</button></div>
    );
  }
  ...
  return (
    <div className="dealsPage">
      ...
      <div className="segmentedTabs dealsTabs">
        ...
        <button type="button" className={filter === "offers" ? "active" : ""} onClick={() => setFilter("offers")}>Торг {pendingCount > 0 && <i>{pendingCount}</i>}</button>
      </div>
      ...
      {!loading && filter === "offers" && (
        <div className="offersLayout">
          <section className="offersColumn">
            <div className="offersHeading"><div><span className="sectionEyebrow">Входящие</span><h2>Тебе предлагают</h2></div><strong>{incoming.filter((offer) => offer.status === "pending").length}</strong></div>
            {incoming.length === 0 && <div className="miniEmpty">На твои лоты пока никто не торгуется.</div>}
            {incoming.map((offer) => {
              const listing = offerListingMap.get(offer.listing_id);
              const buyer = offerProfileMap.get(offer.buyer_id);
              const discount = listing ? Math.round((1 - Number(offer.amount) / Number(listing.price)) * 100) : 0;
              return (
                <article className={`offerCard status-${offer.status}`} key={`in-${offer.id}`}>
                  <div className="offerTop"><span className={`offerStatus ${offer.status}`}>{offerStatusLabel[offer.status]}</span><small>{relativeDate(offer.updated_at)}</small></div>
                  <Link href={`/listing/${offer.listing_id}`}><h3>{listing?.title ?? "Лот"}</h3></Link>
                  <div className="offerMoney"><strong>{rubles(offer.amount)}</strong>{listing && <span>{discount > 0 ? `−${discount}%` : ""} от {rubles(listing.price)}</span>}</div>
                  <div className="offerPerson"><span className="sellerMiniAvatar">{buyer?.first_name?.charAt(0).toUpperCase() ?? "?"}</span><div><strong>{buyer?.first_name ?? "Покупатель"}</strong><small>★ {buyer?.rating ?? 1000} · {buyer?.deals_count ?? 0} сделок</small></div></div>
                  {offer.status === "pending" && <div className="offerActions"><button type="button" className="acceptOffer" disabled={actionId === offer.id} onClick={() => void respond(offer.id, true)}>Принять</button><button type="button" className="declineOffer" disabled={actionId === offer.id} onClick={() => void respond(offer.id, false)}>Отклонить</button></div>}
                </article>
              );
            })}
          </section>
          ...
          <section className="offersColumn">
            <div className="offersHeading"><div><span className="sectionEyebrow">Исходящие</span><h2>Ты предлагаешь</h2></div><strong>{outgoing.filter((offer) => offer.status === "pending").length}</strong></div>
            {outgoing.length === 0 && <div className="miniEmpty">Ты ещё не предлагал свою цену продавцам.</div>}
            {outgoing.map((offer) => {
              const listing = offerListingMap.get(offer.listing_id);
              return (
                <article className={`offerCard status-${offer.status}`} key={`out-${offer.id}`}>
                  <div className="offerTop"><span className={`offerStatus ${offer.status}`}>{offerStatusLabel[offer.status]}</span><small>{relativeDate(offer.updated_at)}</small></div>
                  <Link href={`/listing/${offer.listing_id}`}><h3>{listing?.title ?? "Лот"}</h3></Link>
                  <div className="offerMoney"><strong>{rubles(offer.amount)}</strong>{listing && <span>Цена лота {rubles(listing.price)}</span>}</div>
                  {offer.status === "pending" && <button type="button" className="cancelOfferButton" disabled={actionId === offer.id} onClick={() => void cancelOffer(offer.id)}>Отменить предложение</button>}
                </article>
              );
            })}
          </section>
      ...
      {!loading && filter !== "offers" && !error && visible.length === 0 && <div className="emptyPanel"><div className="emptySymbol">⇄</div><h3>Сделок пока нет</h3><p>Купи первый лот, предложи свою цену или выстави предмет из стартового набора.</p><Link href="/" className="primaryAction">Перейти на рынок</Link></div>}

      {!loading && filter !== "offers" && visible.length > 0 && (
