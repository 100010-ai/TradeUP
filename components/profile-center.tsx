...
export default function ProfileCenter() {
  ...
  if (session.state !== "verified" || !profile) {
    return (
      <div className="authGate compactGate"><div className="authGateIcon">◉</div><span className="sectionEyebrow">Профиль</span><h1>Твоя карьера перекупа</h1><p>Рейтинг, прибыль и статистика появятся после входа через @{session.botUsername}.</p><button type="button" className="primaryAction" onClick={session.openBot}>Открыть TradeUP</button></div>
    );
  }
  ...
  return (
    <div className="profilePage">
      ...
      <section className="walletCard">
        <div><span>Баланс</span><strong>{rubles(profile.balance)}</strong><p>Игровые рубли для сделок</p></div>
        <Link href="/" className="primaryAction">Искать лоты</Link>
      </section>
