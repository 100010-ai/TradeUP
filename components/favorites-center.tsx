...
export default function FavoritesCenter() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    void session.callAction("favorites")
      .then((result) => {
        const typed = result as FavoritesResult;
        setListings((typed.listings ?? []) as MarketListing[]);
      })
  ...
  if (session.state !== "verified" && !loading) {
    return (
      <div className="authGate compactGate">
        <div className="authGateIcon">♡</div><span className="sectionEyebrow">Избранное</span><h1>Сохраняй выгодные лоты</h1><p>Войди через Telegram, и избранное будет привязано к твоему игровому профилю.</p><button type="button" className="primaryAction" onClick={session.openBot}>Открыть TradeUP</button>
      </div>
    );
  }
  ...
  return (
    <div className="collectionPage">
      ...
      {!loading && !error && listings.length === 0 && <div className="emptyPanel"><div className="emptySymbol">♡</div><h3>Пока ничего не сохранено</h3><p>Нажимай сердечко в карточке лота, чтобы не потерять выгодную цену.</p><a href="/" className="primaryAction">Смотреть рынок</a></div>}
