...
export type PhoneNotification = { id: string; type: string; title: string; body: string; href: string | null; created_at: string; read_at: string | null };
...
type TelegramSessionContextValue = {
  ...
  callDepthAction: (action: string, payload?: Record<string, unknown>) => Promise<ActionResult>;
  openBot: () => void;
};
...
function configureTelegramChrome(webApp: NonNullable<Window["Telegram"]>["WebApp"]) {
  try { webApp.setBackgroundColor?.("#000000"); } catch { /* optional */ }
  try { webApp.setHeaderColor?.("#000000"); } catch { /* optional */ }
  try { webApp.setBottomBarColor?.("#000000"); } catch { /* optional */ }
...
export function TelegramSessionProvider({ children }: { children: React.ReactNode }) {
  ...
  const openBot = useCallback(() => {
    ...
    } catch { /* fall through to browser navigation */ }
  ...
  const refresh = useCallback(async () => {
    const webApp = window.Telegram?.WebApp;
    if (!webApp) {
      setState((current) => current === "verified" ? current : "browser");
      return;
    }
    ...
    webApp.expand();
    if (!webApp.initData) {
      setState((current) => current === "verified" ? current : "unavailable");
      return;
    }
  ...
  const refreshUnreadChats = useCallback(async () => {
    ...
    try {
      ...
      // Keep the last known badge during transient network failures.
    } finally {
  ...
  const refreshNotifications = useCallback(async () => {
    ...
    try {
      ...
      // Keep the last known notification state during transient network failures.
    } finally {
  ...
  const callDepthAction = useCallback(async (action: string, payload: Record<string, unknown> = {}) => {
    const result = await request("/api/depth", action, payload);
    if (["bid_auction", "buy_bundle", "create_auction", "create_bundle"].includes(action)) void refresh();
    return result;
  }, [refresh, request]);
  ...
  const value = useMemo<TelegramSessionContextValue>(() => ({
    ...
    callDepthAction,
    openBot,
  }), [state, user, profile, counts, favoriteIds, starterGranted, unreadChats, unreadNotifications, latestNotification, refresh, refreshUnreadChats, refreshNotifications, callAction, callChatAction, callNotificationAction, callDepthAction, openBot]);
