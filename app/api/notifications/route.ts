...
const allowedActions = new Set(["unread", "list", "mark_read", "mark_all"]);
...
export async function POST(request: Request) {
  ...
  if (typeof parsed.initData !== "string" || !parsed.initData || parsed.initData.length > 16_384 || typeof parsed.action !== "string" || !allowedActions.has(parsed.action)) {
  ...
  try {
    ...
    try { result = raw ? JSON.parse(raw) as Record<string, unknown> : { ok: false, error: "empty_notification_response" }; }
    catch { result = { ok: false, error: "invalid_notification_response" }; }
    return NextResponse.json(result, { status: response.status, headers: { "Cache-Control": "no-store" } });
  } catch {
    return NextResponse.json({ ok: false, error: "notification_service_unavailable" }, { status: 502 });
  }
}
